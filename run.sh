#!/usr/bin/env bash
# =============================================================================
# 公共空间细节地图 —— 一键启动 / 复现脚本（B 版：backend/src/modules/appconfig）
#
...
def call(method, path, body=None, token=None):
    ...
    try:
        ...
        try:
            ...
            return err.code, {"raw": raw}, dict(err.headers)
...
def login(account, password):
    status, body, _ = call("POST", "/auth/login", {"account": account, "password": password})
    if status != 200:
        return None, None
    data = body.get("data", {})
...
print("[2/6] 读取配置工作台概览")
status, body, _ = call("GET", "/admin/config", token=admin_token)
overview = body.get("data", {})
base_version = (overview.get("active") or {}).get("version")
step("概览可读", status == 200, f"active=v{base_version}")
step("阈值标签可读", len(overview.get("thresholdLabels") or {}) > 0,
     f"{len(overview.get('thresholdLabels') or {})} 项阈值")

print("[3/6] 草稿 + 结构校验 + 预演")
status, body, _ = call("POST", "/admin/config/draft", {}, token=admin_token)
draft_version = body.get("data", {}).get("draft", {}).get("version")
step("草稿创建/复用成功", status in (200, 201) and isinstance(draft_version, int), f"draft=v{draft_version}")

status, body, _ = call("GET", "/admin/config/draft", token=admin_token)
bundle = body.get("data", {}).get("draft", {}).get("bundle")
step("草稿内容可读", status == 200 and isinstance(bundle, dict) and bool(bundle.get("categories")))

status, body, _ = call("POST", "/admin/config/validate", {"bundle": bundle}, token=admin_token)
step("配置包结构校验通过", status == 200 and body.get("data", {}).get("valid") is True)

status, body, _ = call(
    "POST",
    "/admin/config/preview",
    {"version": draft_version, "viewerUuid": admin_uuid, "role": "admin"},
    token=admin_token,
)
preview = body.get("data", {})
step("预演返回该版本视角", status == 200 and preview.get("effectiveVersion") == draft_version,
     f"effectiveVersion={preview.get('effectiveVersion')}")

print("[4/6] 灰度生效（管理员白名单命中）")
status, body, _ = call(
    "POST",
    "/admin/config/publish",
    {"mode": "canary", "canaryRule": {"percent": 0, "userUuids": [admin_uuid]}, "note": "run.sh demo 灰度"},
    token=admin_token,
)
canary_version = body.get("data", {}).get("version")
step("草稿已发布为灰度", status == 201 and isinstance(canary_version, int), f"canary=v{canary_version}")

status, _, headers = call("GET", "/categories", token=admin_token)
step("管理员请求命中灰度", headers.get("X-Config-Canary") == "1",
     f"X-Config-Canary={headers.get('X-Config-Canary')} X-Config-Version={headers.get('X-Config-Version')}")

user_token, _ = login("user@example.com", PASSWORD)
status, _, headers = call("GET", "/categories", token=user_token)
step("普通用户仍走全量版本", headers.get("X-Config-Canary") == "0",
     f"X-Config-Canary={headers.get('X-Config-Canary')} X-Config-Version={headers.get('X-Config-Version')}")

print("[5/6] 灰度转全量")
status, body, _ = call("POST", "/admin/config/canary/promote", {}, token=admin_token)
promoted = body.get("data", {}).get("version")
step("灰度已提升为全量", status == 200 and promoted == canary_version, f"active=v{promoted}")

print("[6/6] 一键回滚到任意历史版本")
status, body, _ = call(
    "POST",
    "/admin/config/publish",
    {"mode": "full", "fromVersion": base_version, "note": "run.sh demo 回滚"},
    token=admin_token,
)
rolled = body.get("data", {}).get("version")
step("回滚产生新的全量版本", status == 201 and isinstance(rolled, int), f"v{base_version} 的内容 -> v{rolled}")

status, body, _ = call("GET", "/admin/config/versions", token=admin_token)
items = body.get("data", {}).get("items", [])
statuses = {v.get("version"): v.get("status") for v in items}
step("回滚后的新版本处于全量", statuses.get(rolled) == "active")
step("历史版本仍留在档案里", base_version in statuses)
print(f"        版本状态：{statuses}")
