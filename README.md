...
## 维护须知：在线配置（分类 / 属性表单 / 业务阈值）

分类、属性表单与 10 个业务阈值（分页上限、评论编辑窗口、确认冷却期、过期上报阈值、
举报合并窗口、审核锁时长、签名 URL 有效期等）全部在线配置，不再写死在代码里。

配置以**完整配置包**为单位做整体版本化，存储在 `app_config_versions` 表：

- **草稿（draft）**：结构化编辑器改动只落在草稿行，可反复保存；`POST /admin/config/validate`
  做严格结构校验（含阈值安全边界、必填字段一致性、分类代码格式），不合法的配置进不了版本表。
- **草稿预演**：`POST /admin/config/preview` 给定「观看者 UUID / 匿名桶 / 角色」，
  算出该视角下实际可见的分类与阈值，全程不写库。
- **灰度生效（canary）**：按用户 UUID 确定性哈希分桶（0–100%）+ 最多 200 人白名单，
  管理员始终命中灰度。匿名用户用持久化 cookie 分桶，体验稳定。
  请求响应头 `X-Config-Version` / `X-Config-Canary` 标明本次请求用了哪个版本。
- **一键回滚**：任意历史版本（含已归档版本）都可重新全量发布；回滚不修改历史行，
  而是产生一个新版本号，审计链完整。
- **多实例一致性**：进程内缓存 30 秒 TTL 兜底 + Redis pub/sub 失效广播；
  Redis 故障不阻塞，数据库故障用内置默认配置兜底（`modules/appconfig/defaults.ts`）。

灰度隔离的两个关键点：

1. 灰度物化分类表时**只增不改**——灰度版本里的改名/改色/停用不能通过共享的
   `categories` 表泄漏给非灰度用户；"提升为全量"时才做完整物化。
2. 发布前有存量闸门：仍被条目引用的分类不能从配置包删除（只能停用），避免老条目变成孤儿。

配置管理界面在 `/admin/config`（管理员），接口挂在 `/api/v1/admin/config/*`。
全新库由 `npm run seed` 引导出 v1 全量配置。
